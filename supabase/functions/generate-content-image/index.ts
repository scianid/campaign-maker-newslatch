...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!contentItem.image_prompt) {
      return createErrorResponse(
        'No image prompt available',
        'This content item does not have an AI-generated image prompt. Please regenerate the content to include image prompts.',
        400
      );
    }

    console.log('✅ Content item verified, generating image...');
    console.log('📝 Image prompt:', contentItem.image_prompt);

    // Generate image using OpenAI (returns base64)
    const base64Image = await generateImage(contentItem.image_prompt);
